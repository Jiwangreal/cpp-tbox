...
TEST(FdEvent, DisableSelfInReadCallback) {
  ...
  for (auto e : engines) {
    ...
    EXPECT_TRUE(read_event->initialize(read_fd, FdEvent::kReadEvent,
                                       Event::Mode::kPersist));
...
TEST(FdEvent, DisableSelfInWriteCallback) {
  ...
  for (auto e : engines) {
    ...
    EXPECT_TRUE(write_event->initialize(write_fd, FdEvent::kWriteEvent,
                                        Event::Mode::kPersist));
...
TEST(FdEvent, OneWriteMultiRead) {
  ...
  for (auto e : engines) {
    ...
    EXPECT_TRUE(read_event1->initialize(read_fd, FdEvent::kReadEvent,
                                        Event::Mode::kOneshot));
    EXPECT_TRUE(read_event2->initialize(read_fd, FdEvent::kReadEvent,
                                        Event::Mode::kOneshot));
...
TEST(FdEvent, MultiWriteOneRead) {
  ...
  for (auto e : engines) {
    ...
    EXPECT_TRUE(read_event->initialize(read_fd, FdEvent::kReadEvent,
                                       Event::Mode::kPersist));
    EXPECT_TRUE(write_event1->initialize(write_fd, FdEvent::kWriteEvent,
                                         Event::Mode::kOneshot));
    EXPECT_TRUE(write_event2->initialize(write_fd, FdEvent::kWriteEvent,
                                         Event::Mode::kOneshot));
